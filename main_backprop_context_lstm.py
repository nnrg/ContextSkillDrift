...
class ContextModelLSTM(nn.Module):
    def __init__(self, k: int):
        super().__init__()

        # context module
        self.lc = nn.LSTM(k * N_ODOR_CLASSES * FEATURE_SIZE, 10)
...
if __name__ == "__main__":
    ...
    data_folder = f"output/context_lstm_short_harddecay{trial}"
    try:
    ...
    train_context_network(n_epochs=MAX_EPOCHS, T=T, k=5)
    evaluate_network(n_testing_samples=100, T=T, k=5)
